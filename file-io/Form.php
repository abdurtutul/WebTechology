	...
	<?php 
		if ($_SERVER['REQUEST_METHOD'] === "POST") {
			...
			if (!filter_var($email, FILTER_VALIDATE_EMAIL)) 
			{
     	    ...
     	    if ($pass== $cpass)
				{
					 if(empty($fname) or empty($lname) or empty($gname) or empty($dob) or empty($rname) or empty($pname) or empty($pename) or empty($pphone) or empty($email) or empty($url) or empty($username) or empty($pass) or empty($cpass)) 
					{
						...
						$fc = fclose($handle);
						$data = array('id' => $lastIndex + 1, 'fname' => $fname, 'lname' => $lname, 'gname' => $gname, 'dob' => $dob, 'rname' => $rname, 'pname' => $pname, 'pename' => $pename, 'pphone' => $pphone, 'email' => $email, 'url' => $url, 'username' => $username, 'pass' => $pass, 'cpass' => $cpass);
